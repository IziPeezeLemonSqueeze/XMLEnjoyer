<template>
	<q-dialog v-model="XMLViewerStore.GET_DIALOG_MODIFY_TYPE_HIDE">
		<q-card dark
			...
			style="width: -webkit-fill-available;top:-20%; max-height: 500px;">
			<q-toolbar>
				<q-icon size="md"
			...
			<div class="row full-width"
				...
				<q-btn :disable="XMLViewerStore.downloadingMDT"
					dense
					flat
					rounded
					label="Members "
			...
			<q-card-section>
				<q-scroll-area style="height: 200px;max-height: 500px; max-width: 600px;"
					ref="scrollAreaRef">
					<q-list :disable="XMLViewerStore.downloadingMDT"
						bordered
						separator>
						<q-item v-for="(el, index) in XMLViewerStore.nodeOnModify.members"
							...
							<q-item-section>
								<q-popup-edit :disable="XMLViewerStore.downloadingMDT"
									:auto-save="false"
									style="width: 20%"
									v-model="XMLViewerStore.nodeOnModify.members[index]['#text']"
									v-slot="scope">
									<q-input v-if="!AppStore.selectedOrg"
										v-model="scope.value"
										dense
										autofocus
										hint="Premi INVIO per salvare"
										@keyup.enter="scope.set"></q-input>
									<q-select v-if="AppStore.selectedOrg"
										style="height: 90%;z-index:1000; "
										v-model="el['#text']"
										filled
										use-input
										hide-selected
